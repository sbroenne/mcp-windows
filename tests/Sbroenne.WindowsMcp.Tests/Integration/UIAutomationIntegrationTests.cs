...
/// <summary>
/// Integration tests for UI Automation using the test harness window.
...
public sealed class UIAutomationIntegrationTests : IDisposable
{
    ...
    public UIAutomationIntegrationTests(TestHarnessFixture fixture)
    {
        _fixture = fixture;
        _fixture.Reset();
    ...
    {
        // Arrange
        var initialClickCount = _fixture.Form?.ButtonClickCount ?? 0;
        ...
        var newClickCount = _fixture.Form?.ButtonClickCount ?? 0;
    ...
    public async Task FindAndType_InTextBox_EntersText()
    {
        ...
        var actualText = _fixture.Form?.InputText ?? string.Empty;
    ...
    public async Task CombinedWorkflow_TypeAndClick_Succeeds()
    {
        ...
        await Task.Delay(200);
        Assert.Equal("Workflow test", _fixture.Form?.InputText);
        Assert.True((_fixture.Form?.ButtonClickCount ?? 0) > 0, $"Button click count was: {_fixture.Form?.ButtonClickCount ?? 0}");
    }
    ...
    public async Task Find_ListView_ReturnsListControl()
    {
        ...
        Assert.True(result.Success, $"Find failed: {result.ErrorMessage}");
        Assert.NotNull(result.Elements);
        Assert.True(result.Elements.Length >= 1, $"Expected at least 1 list control, found {result.Elements.Length}");
    }
