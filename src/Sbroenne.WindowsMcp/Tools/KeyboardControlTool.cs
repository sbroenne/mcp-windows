...
public sealed partial class KeyboardControlTool : IDisposable
{
    ...
    [Description("Control keyboard input on Windows. Supports type (text), press (key), key_down, key_up, combo, sequence, release_all, and get_keyboard_layout actions. PREFER ui_automation Type action FIRST for text input - it directly sets text in edit controls. Use keyboard_control as fallback when: 1) ui_automation type fails, 2) sending hotkeys/shortcuts, 3) non-standard input fields. IMPORTANT: Use 'expectedWindowTitle' or 'expectedProcessName' to verify the target window BEFORE sending input - the operation will fail with 'wrong_target_window' if the foreground window doesn't match, preventing input from going to the wrong application.")]
