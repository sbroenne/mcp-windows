...
public sealed partial class WindowManagementTool
{
    ...
    [Description("Manage windows on Windows. Supports list, find, activate, get_foreground, get_state, wait_for_state, minimize, maximize, restore, close, move, resize, set_bounds, wait_for, and move_to_monitor actions. WORKFLOW START: This is often the first step - use 'find' to get a window handle, then pass windowHandle to ui_automation for element interaction. BEST PRACTICE: Before sending keyboard/mouse input to a specific window, use 'activate' with the window handle to ensure it has focus. Use 'get_foreground' to verify which window is currently active. Use 'get_state' to check if a window is minimized, maximized, or normal. Use 'wait_for_state' to wait until a window reaches a specific state. TROUBLESHOOTING: If 'find' returns no results, use 'list' to see all available windows and check the exact title. Use regex=true for partial title matching. If window doesn't activate, it may be minimized - use 'restore' first.")]
