...
public sealed partial class ScreenshotControlTool
{
    ...
    [Description("Captures screenshots of screens, monitors, windows, or regions on Windows. USE CASES: 1) Visual verification after actions, 2) Reading content ui_automation cannot access (images, custom rendering), 3) OCR for non-accessible UI. PREFER ui_automation for structured text extraction - it's faster and returns element hierarchy. PREFER capture_annotated (ui_automation action) to discover clickable elements with numbered labels. COORDINATE SYSTEM: Screenshot pixel coordinates = mouse coordinates. If you see a button at pixel (450, 300) in the screenshot, use mouse_control(x=450, y=300, monitorIndex=N). No conversion needed! Targets: 'primary_screen' (main display with taskbar), 'secondary_screen' (other monitor in 2-monitor setups), 'monitor' (by index for 3+ monitors), 'window', 'region', 'all_monitors'. Use 'list_monitors' action to see available monitors. FORMAT TIP: JPEG (default) is optimized for LLM vision - smaller size, faster processing. Use PNG only when you need lossless quality.")]
