...
public sealed partial class ScreenshotControlTool
{
    ...
    [Description("Capture screenshots of screens, monitors, windows, or regions on Windows. Prefer ui_automation for structured UI text; use screenshot_control for visual verification, non-accessible UI, or coordinate-based clicking. Coordinate system: screenshot pixels == mouse coordinates (no conversion). Common flow: (1) ui_automation(capture_annotated) to discover elements, (2) mouse_control(click) using returned clickablePoint, (3) screenshot_control to verify. Window targeting: set target='window' and pass windowHandle as a decimal string copied verbatim from window_management output. Targets: primary_screen, secondary_screen, monitor (monitorIndex), window (windowHandle), region, all_monitors. Tip: JPEG is default and LLM-optimized; use PNG when you need lossless.")]
