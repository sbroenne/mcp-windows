...
public sealed partial class UIAutomationTool
{
    ...
    [Description("Windows UI Automation (UIA): find and interact with UI elements using semantic properties (names, control types, automationId). Framework auto-detection optimizes search depth for each app type (Electron/WinForms/WPF). Recommended workflow: (1) window_management(find/list) to get a window handle, (2) pass that handle verbatim as ui_automation.windowHandle (decimal string), (3) use capture_annotated for discovery, then click/type/toggle by elementId, (4) use screenshot_control for visual verification or OCR. Key idea: prefer elementId-based actions; use mouse_control only as fallback with clickablePoint. Actions: find, get_tree, wait_for, wait_for_disappear, wait_for_state, click, type, select, toggle, ensure_state, invoke, focus, scroll_into_view, get_text, highlight, hide_highlight, ocr, ocr_element, ocr_status, get_element_at_cursor, get_focused_element, get_ancestors, capture_annotated. Electron/Chromium apps: framework auto-detection uses deeper search automatically; element names often come from ARIA labels, so prefer nameContains. OCR: use ocr_element only when get_text is empty (images/custom rendering).")]
