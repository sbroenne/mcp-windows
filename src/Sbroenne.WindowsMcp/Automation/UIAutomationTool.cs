...
public sealed partial class UIAutomationTool
{
    ...
    [Description("Windows UI Automation (UIA): find and interact with UI elements using semantic properties (names, control types, automationId). Ultra-fast tree traversal (~60-130ms) via single COM call with cached properties. Framework auto-detection optimizes search depth (WinForms=5, WPF=10, Electron=15). Workflow: (1) window_management(find/list) for handle, (2) pass handle verbatim as windowHandle, (3) capture_annotated for discovery, (4) click/type/toggle by elementId. Prefer elementId-based actions; use mouse_control only as fallback with clickablePoint. Actions: find, get_tree, wait_for, wait_for_disappear, wait_for_state, click, type, select, toggle, ensure_state, invoke, focus, scroll_into_view, get_text, highlight, hide_highlight, ocr, ocr_element, ocr_status, get_element_at_cursor, get_focused_element, get_ancestors, capture_annotated. Electron/Chromium: auto-detected, uses deeper search and post-hoc filtering; prefer nameContains for ARIA labels. OCR: use ocr_element only when get_text is empty.")]
